...
function Flux:Window(text, bottom,mainclr,toclose)
	...
	SearchFrame.Size = UDim2.new(0, 178, 0, 28)
	...
	-- Filtrar abas ao digitar
	SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
		local query = SearchBox.Text:lower()
		for _, v in next, TabHold:GetChildren() do
			if v:IsA("TextButton") and v.Name == "Tab" then
				local tabTitle = v:FindFirstChildWhichIsA("TextLabel")
				if tabTitle then
					if query == "" or tabTitle.Text:lower():find(query, 1, true) then
						v.Visible = true
					else
						v.Visible = false
					end
	...
	TabHold.Size = UDim2.new(0, 205, 0, 370)
